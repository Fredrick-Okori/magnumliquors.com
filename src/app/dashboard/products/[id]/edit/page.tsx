...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function loadProduct() {
      ...
      try {
        ...
        if (found) {
          ...
          if (isVideo(found.image || "")) {
            setMediaType("video");
            setVideoUrl(found.image || "");
          } else {
            setMediaType("image");
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(`Bottle "${name}" successfully updated!`);
      setTimeout(() => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div>
        <h1 className="text-3xl font-extrabold text-[#18181b] tracking-tight">Edit Bottle Listing</h1>
        <p className="text-xs text-[#71717a] mt-1">
          Update product media, images, showcase videos, pricing, and live inventory.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-white border border-[#e5e5e4] rounded-3xl p-8 shadow-2xs">
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#f4f4f3]">
          ...
          >
            {isSubmitting ? (
              <span>Updating Stock & Syncing...</span>
            ) : (
              <>
                <CheckCircle2 size={16} /> Save Changes & Sync
              </>
            )}
          </button>
        </div>
